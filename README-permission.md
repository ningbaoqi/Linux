...
### 改变安全性设置
#### 改变权限
+ `chmod`命令来改变文件和目录的安全性设置；`chmod options file`或`chmod 760 filename`；使用符号改变权限；格式为：`[ugoa] [+-=] [rwxXs....]`；如：`chomd o+r newfile`将文件newfile赋予读取权限；

|第一个字符定义了权限作用的对象|第二个字符含义|第三个字符含义|
|------|------|------|
|`u`代表用户|`+`在现有权限基础上增加权限|`X`如果对象是目录或者它已有执行权限，赋予执行权限|
|`g`代表组|`-`在现有权限的基础上减少权限|`s`运行时重新设置UID和GID|
|`o`代表其他|`=`将权限设置成后面的值|`t`保留文件或目录|
|`a`代表以上全部||`u`将权限设置为跟属主一样|
|||`g`将权限设置为跟属组一样|
|||`o`将权限设置为跟其他用户一样|

+ `chmod`的`-R`选项可以让权限的改变递归地作用到文件和子目录，可以使用通配符指定多个文件，然后一起设置权限；

### 改变所属关系
 + 改变文件的属主可以使用`chown`命令，改变文件的默认属组使用`chgrp`命令；
#### chown
+ 可以使用登录名或UID来指定文件的新属主 如：`chown ningbaoqi newfile`；可以同时改变文件的属主和属组 如 `chown ningbaoqi.test newfile`；可以只改变一个目录的默认属组 如：`chown .test newfile`；chown 的`-R`选项配合通配符可以递归的改变子目录和文件的所属关系，`-h`选项可以改变文件的所有符号链接文件的所属关系；只有root用户能够改变文件的属主，任何属主都可以改变文件的属组，但前提是属主必须是原属组和目标属组的成员；
#### chgrp
+ 改变文件或目录的默认属组 `chgrp ningbaoqi newfile`；
