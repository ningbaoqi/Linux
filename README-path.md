...
### 数组变量
+ 要给某个环境变量设置多个值，可以把值放在括号内，值与值之间以空格分隔`mytest=(one two three four five),`但是在`echo $mtytest`的时候只会输出第一个值one；引用单独的数组元素，必须用代表它在数组中位置的数值索引值，索引值要用方括号括起来` echo ${mytest[1]} `将输出two；显示整个数组变量，使用星号通配符放在索引值的位置；`echo ${mytest[*]}`；改变某个索引值位置的值 `mytest[2]=sb`；删除数组中的某个值：`unset mytest[2]` ,只是删除了第三个的值，但是这个数组的长度没有改变，只是第三个元素没有值；删除整个数组：`unset mytest` 也是删除了值；
