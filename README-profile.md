...
### 理解shell的内建命令
#### 外部命令
+ 外部命令也被称为文件系统命令，存在于bash shell之外的程序，他们并不是shell程序的一部分，外部命令通常位于/bin、/usr/bin、/sbin、/usr/sbin中；ps就是一个外部命令；可以使用which和type命令找到它；如：`which ps` 就可以找到（which只显示外部命令）；`type -a ps`（会显示外部命令和内建命令）；当外部命令执行时，会创建一个子进程，被称为衍生（fork）使用ps进行查看；
#### 内建命令
+ 内建命令不需要使用子进程来执行；cd 和 exit 命令都内建于bash shell 使用type命令了解是不是内建命令：`type cd`；`type exit`；
#### 特殊命令
+ echo和pwd即有内建命令也有外部命令，可以使用`type -a `命令来查看，要使用外部命令pwd,可以输入`/bin/pwd`；
#### history命令
+ 通常历史记录中会保存最近的1000条命令，可以设置保存在bash历史记录中的命令数量，要想实现这一点，需要修改名为HISTSIZE的环境变量；可以换回并重用历史列表中最近的命令，使用两个感叹号就可以执行上一条命令 `！！`；命令历史被保存在隐藏文件.bash_history中，它位于用户的主目录，bash命令的历史记录是先存放在内存中，当shell退出时再被写入到历史文件中的；可以在shell退出之前强制将目录历史写入到目录历史文件中，使用命令 `history -a`；.bash_history文件只有在打开首个终端会话时才会被读取，要想强制重新读取.bash_history文件，更新终端会话的历史记录，可以使用`history -n`命令；使用 `！命令ID`就可以使用该条历史命令；
#### 命令别名 
+ alias命令是另一个shell的内建命令，命令别名允许你为常用的命令创建另一个名称；查看当前可用的别名：`alias -p`；创建自己的别名：`alias li='ls -li' `然后就可以使用 li 别名命令了；创建好别名命令之后，只能在它所被定义的shell进程中才有效，就算在shell脚本中也没问题，要注意，因为命令别名属于内部命令，一个别名仅在它所被定义的shell进程中才有效；
